...
> 来源阶段：第二阶段页面流转设计
> 文档状态：长期页面与导航边界；课程期简化项仅作历史说明
> 最近更新：2026-07-14

## 1. 目标

本页面架构最初用于支撑第二阶段课程提交和 Compose UI 搭建。页面骨架现已完成；文中
“第二阶段可简化为”保留为设计演进记录，不代表当前实现仍是 mock 或占位。本文固定：
...
- 本地可读：用户导入的 `.kmd` / `.kmdwork` 或已有本地 source 的作品。
- 需联网：只有元数据或收藏关系、没有本地 source 的社区作品。
- 导入入口：添加本地 `.kmd` 到书架。
- 工具入口：导入、设置和关于。排序/筛选/移出书架属于 deferred；删除和缓存清理尚无数据生命周期授权。

截至 R3-K1 的实现基线：

- ✅ 本地导入按钮（书架底部 + 空书架占位 CTA）。
...
- ✅ 全局阅读偏好（R3-I）：SettingsSheet + Reader chrome 入口、DataStore 持久化和 runtime settings 投影。
- ✅ R3-K1：纯书架 UI 投影、Library/History 模式切换、continue、本地可读/需联网分组、导入状态和响应式单主滚动布局已实现并完成视觉验收。
- Deferred：排序筛选、非破坏性管理与笔记/书签只记录在 Post-R3 backlog；删除/缓存动作仍受独立数据生命周期 gate 约束。
...
页面骨架、书架、详情、阅读、导入、设置与 companion 均已完成本阶段实现。本文件不再维护课程
截图清单或旧 Compose 原型顺序；历史方案与证据见
[`../../archive/course-r3-2026/`](../../archive/course-r3-2026/README.md)，未来工作从
[`../../planning/post-r3-reentry-backlog.md`](../../planning/post-r3-reentry-backlog.md) 重新立项。
