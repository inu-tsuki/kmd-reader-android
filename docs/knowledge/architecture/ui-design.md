...
- 本地可读：导入的 `.kmd` / `.kmdwork` 或已有本地 source 的作品，强调离线可读。
- 需联网：只有元数据但缺少本地 source 的收藏作品，明确标注“需联网”。
- 状态：空书架、空历史、导入中和导入失败都有明确反馈。当前模型不伪造“已缓存社区作品”分类。

视觉上，书架应接近阅读应用的图书馆：密度适中、便于扫描、重视标题和进度；不要把它做成账号页、项目介绍页或设置页。项目说明可以保留在底部或更多菜单里，不能压过作品列表。

实现状态：R3-F/G/H 与 R3-K1 已完成书架骨架、收藏/续读、信息层级、响应式状态和视觉验收。
排序筛选与管理动作不再属于活跃 R3 切片，未来按
[`Post-R3 重启 Backlog`](../../planning/post-r3-reentry-backlog.md) 重新评估；删除和缓存仍必须先定义数据生命周期。
