...
> 文档状态：历史执行记录；R3 已以 A–I + K1 为完成边界封存
> 最近更新：2026-07-14
> 代号：R3
...
R3 完成标准：不依赖任何后端，用户可以导入 `.kmd` / `.kmdwork`、在书架管理、阅读并恢复进度、
调整阅读偏好，并让 issue draft 本地防丢。笔记/书签为可选增强，close/reopen 与正式提交属于未来社区语义。
...
R3-F/G/H 交付功能骨架后，独立收束书架的信息层级、响应式表现、状态表达与管理动作。K1
K1 已完成并通过复审：建立纯 UI 投影，完成继续阅读、现有事实可表达的本地可读/需联网分组、书架/历史
视图、导入状态和多尺寸适配。K2 可再做排序筛选与非破坏性管理；删除/缓存需先过
独立数据边界 gate。详见 [`r3-k-bookshelf-experience-plan.md`](r3-k-bookshelf-experience-plan.md)。
...
R3-A 数据层（entry + revision + drafts [+ annotation]，无依赖）
  ...
  ├─→ R3-I 设置页（阅读偏好，独立无依赖）
  ├─→ R3-J 笔记/书签（视精力）
  └─→ R3-K 书架体验收束（依赖 F/G/H，建议 I 后实施）
```

已完成：A → B → C → D → E → F → G → H → I → K1。

封存结论：R3 以 K1 为完成边界。K2、J、R1/R2 余项均进入 Post-R3 backlog，恢复时重新立项。
...
- 播放到中段退出 → 重新进入 → 恢复进度。
- 详情页有进度时显示「继续阅读」+ 位置摘要。
- issue draft 退出阅读后不丢；close/reopen 与正式提交不属于 R3 持久化验收。
