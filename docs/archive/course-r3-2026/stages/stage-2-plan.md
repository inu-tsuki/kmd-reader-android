...
其中收藏持久化、完整设置页、完整评论区、真实登录和真实上传暂不作为第二阶段 MVP；书架页面保留为主页面，但第二阶段可以只实现本地/示例作品和导入入口占位。
...
| 书架页 | 管理本地/收藏/最近阅读作品，包含导入、设置和项目说明入口，第二阶段可简化 |
...
第二阶段后续设计调整为活动桌面式导航，不再推荐独立底部导航。主页面为：

- `书架`：本地导入、最近阅读、收藏/缓存、设置和项目说明入口。
- `发现`：发现流、搜索、作品详情入口。

审核不作为一级 Tab，而是从作品详情或阅读页进入检查/审阅面板。阅读器页、作品详情页、搜索页和审核详情页作为二级页面或浮层，不放在主导航中。
...
- 设计方向：用 Figma 画书架页、发现流页、作品详情页、阅读器页、审核队列页、审核详情页。
