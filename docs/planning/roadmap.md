...
- Runtime bridge：`loadScript -> ready -> play/pause/seek -> progress` 最小链路已闭合。
...
- `ReviewOverlay` 还没有迁入 `ReaderCompanionContainer`。
- 审阅没有 `KmdSourceSnapshot`、源码片段、issue 定位和轻度修改建议。
...
| ReviewOverlay 仍是全局浮窗 | 已识别 | UI-4 建立 `ReaderCompanionContainer` 后迁入 |
...
1. **R1-1 错误恢复收束**：让 renderer 退出、source missing、runtime failed 都有统一恢复 UI。
2. **R2-4A Companion State**：新增状态和 reducer，不动 UI。
3. **R2-4B 空 Companion 容器**：挂到 `ReaderDesk`，验证 WebView 不重建。
4. **R2-4C Review 迁移**：把现有审阅浮窗搬进 companion。
5. **R2-3 横屏观看按钮**：先按钮和提示，再考虑 Activity orientation lock。
