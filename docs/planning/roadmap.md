...
3. **R2-4H 手测**：极简 command tray、源码行气泡、重复滚动和关闭手感。
4. **R2-4G 手测**：Review / Issues 切换时 WebView 不重建、companion 高度不跳。
5. **R2-4E 手测**：bottom sheet / side panel / shared height。
6. **R2-3 横屏观看按钮**：先按钮和提示，再考虑 Activity orientation lock。
