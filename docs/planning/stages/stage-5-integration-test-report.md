...
| 编号 | 验证项 | 命令或位置 | 当前记录 |
|------|--------|------------|----------|
| AV-01 | Reader Runtime Web 构建 | `pnpm reader:build` | 已通过 |
| AV-02 | 主仓库 TypeScript/Vite 构建 | `pnpm build` | 已通过 |
| AV-03 | Android Debug APK 编译 | `./gradlew :app:assembleDebug` | 已通过，2026-05-20 重跑 |
| AV-04 | Android 单元测试 | `./gradlew :app:testDebugUnitTest` | 已通过，2026-05-20 重跑 |
...
| 后端服务 | `kmd-community-api`，本机 `http://10.0.2.2:3000` |
| Runtime 产物 | `dist/reader-runtime/` 同步到 APK assets |
...
|----------|------|----------|----------|----------|----------|------|
| IT-01 | 启动与作品列表加载 | 1. 启动 App；2. 等待首页加载完成 | App 不闪退；展示作品列表或明确的加载/错误状态 | 待手动执行 | 待填写 |  |
| IT-02 | 列表进入详情再返回 | 1. 点击第一个作品；2. 查看详情；3. 返回列表；4. 点击第二个作品 | 详情页内容与点击作品一致；返回后列表仍可操作 | 待手动执行 | 待填写 |  |
| IT-03 | 详情进入阅读页 | 1. 进入作品详情；2. 点击阅读按钮 | 进入阅读页；WebView runtime 正常加载；无黑屏占位 | 待手动执行 | 待填写 |  |
| IT-04 | KMD 脚本播放 | 1. 阅读页等待加载；2. 开始播放；3. 观察 20 秒 | 文字或画面正常显示；进度变化；播放期间不崩溃 | 待手动执行 | 待填写 |  |
...
| IT-07 | 后端不可用时的兜底 | 1. 关闭 `kmd-community-api`；2. 冷启动 App；3. 进入列表/详情 | App 不闪退；展示缓存、mock 数据或错误提示 | 待手动执行 | 待填写 |  |
| IT-08 | 屏幕旋转稳定性 | 1. 在列表、详情、阅读页分别旋转屏幕；2. 每页旋转 3 次 | 不闪退；页面可继续操作；阅读页能恢复或给出明确错误 | 待手动执行 | 待填写 |  |

## 5. Bug 清单与修复记录

修复率计算方式：已修复或已缓解的 Bug 数 / 已发现 Bug 总数。当前记录 8 个问题，其中 7 个已修复或已缓解，修复率为 88%。
